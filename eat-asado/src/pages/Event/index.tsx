                    ...
                    <section className={styles.event} data-testid="event-main-section">
                                        ...
                                        <div className={styles.chefDesigneeSection}>
                                            ...
                                            <div className={styles.assignTransitionWrapper} data-testid="event-chef-assign-wrapper">
                                                ...
                                                {!event.chef && event.state === EventStatesEnum.AVAILABLE && isUserIntoEvent() && (
                                                    <AssignBtn key={`assign-${user?.id}`} kind="assign" style={isLoading || isSyncingEvent ? { opacity: 0.5 } : {}} disabled={isLoading || isSyncingEvent} onClick={() => toogleChef()}></AssignBtn>
                                                )}
                                            </div>
                                        </div>
                                    </div>
                                        ...
                                        <div className={styles.shoppingDesigneeDescSection}>
                                            ...
                                            <div className={styles.assignTransitionWrapper} data-testid="event-shopdesignee-assign-wrapper">
                                                {!event.shoppingDesignee.length && event.state === EventStatesEnum.AVAILABLE && isUserIntoEvent() && (
                                                    <AssignBtn key="assign" kind="assign" style={isLoading || isSyncingEvent ? { opacity: 0.5 } : {}} disabled={isLoading || isSyncingEvent} onClick={() => toogleShopDesignee()}></AssignBtn>
                                                )}
                                                {Boolean(
                                                ...
                                                ) && <AssignBtn key="unassign" kind="unAssign" style={isLoading || isSyncingEvent ? { opacity: 0.5 } : {}} disabled={isLoading || isSyncingEvent} onClick={() => toogleShopDesignee()}></AssignBtn>}
                                                {event.shoppingDesignee.length > 0 &&
                                                    event.state === EventStatesEnum.AVAILABLE &&
                                                    isUserIntoEvent() &&
                                                    !event.shoppingDesignee.some((d: IUser) => d._id === user?.id) && (
                                                        <AssignBtn key="add" kind="add" style={isLoading || isSyncingEvent ? { opacity: 0.5 } : {}} disabled={isLoading || isSyncingEvent} onClick={() => toogleShopDesignee()}></AssignBtn>
                                                    )}
                                            </div>
                                        </div>
                        ...
                        <section className={styles.btnSection} data-testid="event-main-btn-section">
                            {event.state === EventStatesEnum.AVAILABLE && !isLoading && (
                                <div data-testid="event-participation-btn-wrapper">
                                    {!isUserIntoEvent() && isEventFull() ? (
                                        ...
                                        <Button className={styles.btnEvent} kind="secondary" size="short" onClick={() => toogleParticipation()} disabled={isLoading || isSyncingEvent}>
                            ...
                            {event.organizer && event.organizer?._id === user?.id && (
                                <Button className={styles.btnEvent} kind="secondary" size="short" onClick={() => deleteTheEvent()} disabled={isLoading || isSyncingEvent}>
                                    {lang.deleteEventBtn}
                                </Button>
                            )}
                            {event.organizer &&
                                ...
                                event.purchaseReceipts.length > 0 && (
                                    <Button className={styles.btnEvent} kind="secondary" size="short" onClick={() => setEventToReadyForPay()} disabled={isLoading || isSyncingEvent}>
                            ...
                            {event.organizer &&
                                ...
                                event.state !== 'finished' &&
                                event.state !== EventStatesEnum.CLOSED && (
                                    <Button className={styles.btnEvent} kind="secondary" size="short" onClick={() => closeEvent()} disabled={isLoading || isSyncingEvent}>
                                        {lang.closeEventBtn}
                                    </Button>
                                )}
                            {event.organizer &&
                                ...
                                (event.organizer?._id === user?.id || event.shoppingDesignee.some((d: IUser) => d._id === user?.id)) &&
                                event.state === EventStatesEnum.CLOSED && (
                                    <Button className={styles.btnEvent} kind="secondary" size="short" onClick={() => reopenEvent()} disabled={isLoading || isSyncingEvent}>
                                        {lang.reopenEventBtn}
                                    </Button>
                                )}
                            {event.shoppingDesignee &&
                                ...
                                (!checkIfUserHasUploaded() ? (
                                    <Button className={styles.btnEvent} kind="primary" size="short" onClick={() => payCheck()} disabled={isLoading || isSyncingEvent}>
                                        {lang.payBtn}
                                    </Button>
                                ) : (
                                    !checkIfUserHasPaid() && (
                                        <Button className={styles.btnEvent} kind="secondary" size="short" onClick={() => payCheck()} disabled={isLoading || isSyncingEvent}>
                                            {lang.modifyPay}
                                        </Button>
                                    )
                                ))}
                            {event.shoppingDesignee &&
                                event.shoppingDesignee.some((d: IUser) => d._id === user?.id) &&
                                event.state === EventStatesEnum.CLOSED && (
                                    <Button className={styles.btnEvent} kind="primary" size="short" onClick={() => openModalPurchaseRecipt()} disabled={isLoading || isSyncingEvent}>
                                        {lang.loadPurchase}
                                    </Button>
                                )}
                        </section>
